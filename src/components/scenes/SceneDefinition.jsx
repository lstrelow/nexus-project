      ...
      {phase === "def" && (
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(140px,1fr))", gap: 10, marginBottom: 14 }}>
            {d.aspects.map((a, i) => (
              <div key={i} style={{ padding: "12px 14px", borderRadius: 12, background: C.bgWarm, border: `1px solid ${C.border}` }}>
                <div style={{ fontSize: 20, marginBottom: 6 }}>{a.icon}</div>
                <div style={{ color: C.text, fontSize: 12, fontWeight: 700, marginBottom: 3 }}>{a.title}</div>
                <div style={{ color: C.textMid, fontSize: 11, lineHeight: 1.5 }}>{a.text}</div>
              </div>
            ))}
          </div>

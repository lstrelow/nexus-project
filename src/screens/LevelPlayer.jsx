...
import { Avatar, XPBar, SpeechBubble, NextBtn } from "../components/ui/index.jsx";
...
import { SceneChartPicker } from "../components/scenes/SceneChartPicker.jsx";
    ...
    <div style={{ minHeight: "100vh", background: C.bg }}>
      ...
      <div style={{ background: C.bgCard, borderBottom: `1px solid ${C.border}`, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 10, boxShadow: "0 1px 8px rgba(0,0,0,0.05)" }}>
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ color: C.text, fontSize: 13, fontWeight: 700 }}>{MODULES[0].levels[levelNr - 1].title}</div>
        </div>
        <XPBar xp={xp} />
      </div>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "20px 16px" }}>
        ...
        {phase === "scene" && (
          <div style={{ display: "flex", gap: 4, marginBottom: 22 }}>
            {data.scenes.map((_, i) => (
              <div key={i} style={{ flex: 1, height: 4, borderRadius: 99, background: i <= idx ? C.accent : C.bgDeep, transition: "background 0.3s" }} />
            ))}
          </div>
        )}
        ...
        {phase === "intro" && (
          <div>
            ...
            <div style={{ padding: "14px 18px", borderRadius: 14, background: C.bgWarm, border: `1px solid ${C.border}`, marginBottom: 20 }}>
              ...
              {(LEARNING_GOALS[levelNr] || []).map((t, i) => (
                <p key={i} style={{ color: C.textMid, fontSize: 13, margin: "4px 0" }}>• {t}</p>
              ))}
            </div>
        ...
        {phase === "scene" && scene && (
          <>
            ...
            {scene.type === "chartpicker" && <SceneChartPicker key={scene.id} scene={scene} onNext={next} onStats={handleStats} />}
        ...
        {phase === "complete" && (
          <div style={{ padding: "32px 0" }}>
            <div style={{ textAlign: "center", marginBottom: 20 }}>
              ...
              <h2 style={{ color: C.text, fontSize: 26, fontWeight: 900, margin: 0 }}>Level abgeschlossen!</h2>
            </div>

            <div style={{ display: "flex", gap: 12, marginBottom: 16, textAlign: "left" }}>
              <div style={{ flex: 1, padding: "16px", borderRadius: 16, background: C.accent + "15", border: `1.5px solid ${C.accent}44`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div style={{ color: C.accent, fontSize: "clamp(20px,5vw,26px)", fontWeight: 900 }}>+{data.outro.xp} XP</div>
              </div>
              <div style={{ flex: 1, padding: "16px", borderRadius: 14, background: C.bgWarm, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ padding: "14px 18px", borderRadius: 14, background: C.bgWarm, border: `1px solid ${C.border}`, display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 24, textAlign: "left" }}>
            ...
            <button
              onClick={() => onComplete(data.outro.xp, { decisions, correct, total: decisions })}
              style={{ width: "100%", padding: "14px", borderRadius: 14, background: C.accent, color: "#fff", fontSize: 15, fontWeight: 800, border: "none", cursor: "pointer", fontFamily: "inherit", boxShadow: `0 4px 20px ${C.accent}44` }}
            >
              Nächstes Level →
            </button>
          </div>
